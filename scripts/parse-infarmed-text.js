...
const USAGE = `Usage:
  node scripts/parse-infarmed-text.js <medId> [--out <path>] [--infarmed-id <id>] [--best-match]

Parses .txt files produced by pdftotext and extracts sections
from RCM and FI documents.
`;
...
function collectTxtFiles(rootDir) {
...
function resolveInfarmedId(medDir, args) {
  const explicitIndex = args.indexOf('--infarmed-id');
  if (explicitIndex !== -1) {
    return args[explicitIndex + 1] || null;
  }

  if (args.includes('--best-match')) {
...
function main() {
  ...
  const infarmedId = resolveInfarmedId(medDir, args);
  const txtFiles = filterByInfarmedId(collectTxtFiles(medDir), infarmedId);
  if (txtFiles.length === 0) {
    console.log(`No .txt files found in ${medDir}. Run pdftotext first.`);
    return;
  }
